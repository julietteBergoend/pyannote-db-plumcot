...
def clearDataTXT():
    ...
    for fileName, fileContent in filesTXT.items():
        ...
        regex = r'(\<.*?\>)|(\[.*?\])|(\(.*?\))'    
        fileContent = re.sub(regex, '', fileContent) # Retirer ce qui est entre <>, () et []
        ...
        if "TheWalkingDead" in fileName or "TheOffice" in fileName:
            ...
            fileContent = fileContent.replace('(adsbygoogle = window.adsbygoogle || ).push({});', '') # Retirer un message automatique
            emptyLinesRemoved = " ".join(fileContent.split()) # Retirer les lignes vides
            fileContent = re.sub(locIdentification, r'\n\1\2\3', emptyLinesRemoved) # Mettre les locuteurs en début de ligne
            fileContent = re.sub(automaticMessage, '', fileContent) # Retirer un message automatique
            fileContent = re.sub(automaticMessage2, '', fileContent) # Retirer un message automatique
            fileContent = re.sub(spaceNamePunct, r'\1 \2', fileContent) # Mettre un espace entre le nom et les :
            fileContent = re.sub(addWhitespace, r'\1\2 \3', fileContent) # Mettre un espace entre une ponctuation et le mot suivant
            fileContent = fileContent.replace('  ', ' ') # Transformer les doubles espaces en un seul espace
            fileContent = re.sub(beforeScript, '', fileContent) # Retirer un message automatique
...
def finalDataTXT():
    ...
    for fileName, fileContent in filesTXT.items():
        ...
        fileContent = fileContent.replace('  ', ' ')
        with codecs.open ("txt_final/"+fileName.split(".")[0]+"/"+fileName+".temp",'w', "utf8") as f:
             f.write(fileContent)
...
def clearDataTemp():
    ...
    for fileName, fileContent in filesTEMP.items():
        if "TheWalkingDead.Season05.Episode14" in fileName:
            fileContent = fileContent.replace('<div class="scrolling-script-container">\r\n\r\n', "")
            
            fileContent = fileContent.replace("<br/> ", "\n")
            fileContent = fileContent.replace("<br> ", "\n")
            fileContent = re.sub(r'(<br>)(\n)(</br>)*(</div>)', '', fileContent) # retirer les balises html
            fileContent = re.sub(r'(.*)(\-)(.*)', r'\1\n\2\3', fileContent) # enonces qui commencent par - : retirer le tiret et mettre a la ligne
            fileContent = fileContent.replace('- ', '').replace('-', '')
            fileContent = fileContent.replace('''<br>'" ''', '\n').replace("<br/>", "") # retirer les balises html
            fileContent = re.sub(r'(\{.*\} )|(\[.*\])|(\(.*\) )' , '', fileContent) #retirer ce qui est contenu entre parentheses et crochets
            fileContent = fileContent.replace("   <br/>\n", "").replace("</div>", "").replace('<br>"', '"\n').replace("<br>", "") # retirer les balises html
            fileContent = fileContent.replace("Previously on AMC's The Walking Dead", "").replace('Previously on AMC\'s "The Walking Dead" ', "").replace('Previously on AMC\'s "The Walking Dead ', "")
            fileContent = re.sub(r'( ){2,}', " ", fileContent) # enlever les espaces en trop
    
            fileContent = re.sub(identifiedLoc, r"\n\1\2\3\4", fileContent) # quand on a 2 loc par enonce : mise a la ligne
    
            # ajout du faux nom de locuteur not_available
            fileContent = 'not_available '.join(fileContent.splitlines(True))
            fileContent = re.sub(r'(			 |			)', "", fileContent) # retirer des espaces precis
            
            
            
            fileContent = re.sub(identifiedLoc2, r"\2\3\4\5 ", fileContent) # lorsqu'on a le nom du locuteur on ne met pas "not-available"
            ...
            with codecs.open ("temp_cleaned/"+fileName.split(".")[0]+"/"+fileName+".txt",'w', "utf8") as f:
                 f.write(fileContent)
