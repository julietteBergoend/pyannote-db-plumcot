        ...
        if os.path.isdir(path.join(directory, foldername)):
           for filename in os.listdir(path.join(directory, foldername)):
               ...
               if ext == TXT and SERIE in filename and SEASON in filename:
                   with open(path.join(directory, foldername, filename), 'r', encoding='utf8',errors='ignore') as file:
                       texts[str(directory)+str(foldername)+"/"+str(basename)] = file.read()    
    ...
    for filename, fileContent in files.items():
                ...
                if not "#" in line.split()[0]:
                    if "_" in line.split()[0]:
                        ...
                        if lastName not in names:
                            names.append(lastName) # ajout des noms normalisés dans une liste
                else:
                    name = line.split()[0].split('#')[0]
                    names.append(name)
                    
        names2 = [item for item in names if not "." in item and item != "he" and item != "emily_" and item != "should" and item != "all" and item != "sir" and item != "portrait_" and item != "old" and item != "man" and item != "REDSHIRT_" and item != "professor" and item != "mr" and item != "the" and item != "of"]
        ...
        for j in range(len(sentences)):    
            ...
            if "#" in sentence : # voice_from_buzzer#unknown#TheBigBangTheory.Season01.Episode01
                sentence = sentence.replace(sentence.split(" ")[0], sentence.split(" ")[0].split("#")[0])
            ...
            # Spacy    
            doc = nlp(sentence)
            listTuples = [(token.text, token.pos_, token.tag_, token.dep_) for token in doc] 
            ...
            for k in range(len(newListTuples)):
                ...
                sentenceSepPunct = re.sub(r'([A-Za-z]{1}) (-) (.*)( )', r'\1\2\3\4', sentenceSepPunct)
                sentenceSepPunct = re.sub(r'(Mr|Mrs|¾|Ms) (\.)', r'\1\2', sentenceSepPunct)
                ...
                if word != newListTuples[0][0]: # si le mot n'est pas le premer mot (nom normalisé du locuteur)
                    iterWord += 1
                    ...
                    if wordLower in names2:
                    ...
                    elif wordLower in ["you", "ya", "your", "yourself", "yours"] and j-1 >= 0:
                        locSentBefore = nltk.word_tokenize(sentences[j-1])[0] # nom speaker phrase -1
                        if j < len(sentences)-1:
                            locSentAfter  = nltk.word_tokenize(sentences[j+1])[0] # nom speaker phrase +1
                        ...
                        if locSentBefore == locSentAfter: # si le loc de la phrase d'avant et après est le même
                            iterEntity += 1 
                            dataframe['referenceTo'].append(locSentAfter) # le you / your fait reference à cette personne
                            dataframe['idEntity'].append(iterEntity)
                            
                        elif locSentBefore != locSentAfter and newListTuples[0][0] != locSentBefore and newListTuples[0][0] != locSentAfter:
                            if newListTuples[k+1][0].lower() in names2 : # si le mot qui suit "you" est dans la liste des noms
                                iterEntity += 1 
                                dataframe['referenceTo'].append(newListTuples[k+1][0].lower()) # le "you" désigne cette personne
                                dataframe['idEntity'].append(iterEntity)
                                
                            elif sentence.endswith('?') and newListTuples[k-1][0].lower() in ['do', 'did'] or newListTuples[k+1][0].lower() in ['do', 'did'] and newListTuples[k-1][0].lower() != "guys": # si la phrase courante est une question et que "you" est précédé de "do"
                                iterEntity += 1 
                                dataframe['referenceTo'].append(locSentAfter) # on imagine que la personne va répondre donc est la référence au you
                                dataframe['idEntity'].append(iterEntity)
                    ...
                    elif wordLower in ["mommy", "mom", "daddy", "dad", "papa", "mother", "father"]:
                        ...
                        elif newListTuples[k-1][0].lower() not in ["your", "'s", "his", "her", "our"]:
                            if word.lower() in ["father", "dad", "daddy", "papa"]:
                                for elem in masculin_parents:
                                    if newListTuples[0][0].split("_")[-1] == elem.split("_")[-1]:
                                        iterEntity += 1
                                        ...
                                        iterEntity += 1
                                        dataframe['referenceTo'].append(newListTuples[0][0]+"'s_father")
                                        dataframe['idEntity'].append(iterEntity)
                                        break
                            else:
                                for elem in feminin_parents:
                                    if newListTuples[0][0].split("_")[-1] == elem.split("_")[-1]:
                                        ...
                                        iterEntity += 1
                                        dataframe['referenceTo'].append(newListTuples[0][0]+"'s_mother")
                                        dataframe['idEntity'].append(iterEntity)
                                        break
                        else:
                            iterEntity += 1 
                        ...
                        if newListTuples[k-1][0].lower() == "my":
                            iterEntity += 1
                            dataframe['referenceTo'].append(newListTuples[0][0]+"'s_parents")
                            dataframe['idEntity'].append(iterEntity) 
                        else:
                            iterEntity += 1 
                            dataframe['referenceTo'].append('PARENTS')    
            ...
            for index, row in df.iterrows():
                ...
                for name in names2:
                    if "_" in name:
                        if str(val).lower() == name.split("_")[0] or str(val).lower() == name.split("_")[1]:
        ...
        # ECRITURE
        print(filename, "OK")
        df.to_csv(PATH_CSV+"semi-auto-annot_"+filename+".conll", sep='\t', encoding='utf-8', index=False, header=False) 
        df.to_csv(PATH_CSV+"semi-auto-annot_"+filename+".csv", sep=';', encoding='utf-8') 
    ...
    for filename, fileContent in jsonFiles.items():
        if JSON in filename and SERIE in filename:
            ...
            for index, row in df.iterrows():
                ...
                for label in labels:
                    if label[0] == 0:
                        idCharLabels.append([label[0], label[1]-len(text.split(' ')[0])-1, label[2]])
                        listIdChar.append([label[0], label[1]-len(text.split(' ')[0])-1])
                        listLabels.append(label[2])
                    else:
                        idCharLabels.append([label[0]-len(text.split(' ')[0])-1, label[1]-len(text.split(' ')[0])-1, label[2]])
                        listIdChar.append([label[0]-len(text.split(' ')[0])-1, label[1]-len(text.split(' ')[0])-1])
                        listLabels.append(label[2])

                # If 0 entity
                if len(labels) == 0:
                    df2 = df2.append({'idSent' : index+1, 'idChar' : "" , 'label' : ""} , ignore_index=True)
                # If 1 entity    
                elif len(labels) == 1:
                    for l, c in zip(listLabels, listIdChar):
                        df2 = df2.append({'idSent' : index+1, 'idChar' : c , 'label' : l} , ignore_index=True)
...
def stats():
    ...
    dfStats = pd.DataFrame(columns=['Serie', 'Nb episodes', 'Nb phrases', 'Nb annotations', 'Noms', 'Pronoms', 'Pronoms poss', 'Noms propres', 'multiple_persons', 'Autres', 'Auto=Manuel', 'Auto!=Manuel'])
    series = ["StarWars", "TheBigBangTheory", "TheWalkingDead", "HarryPotter", "GameOfThrones", "TheOffice", "BuffyTheVampireSlayer", "Lost", "BreakingBad", "Friends", "BattletarGalactica"]
    ...
    for serie in series:
        ...
        dfStats = dfStats.append({'Serie' : serie, 
                                  ...
                                  'Auto!=Manuel' : annotation_differente}, ignore_index=True)

    dfStats.to_csv("/people/lefevre/Bureau/projet/dev/Stat_Annotations.ods", sep='\t', encoding='utf-8')
