...
def extractSpringfieldWebsite():
    ...
    for serie in series_no_loc:
        ...
        soup_page_web2 = BeautifulSoup(html_page2, 'html.parser') # parsing du html avec bs4
        data2 = soup_page_web2.findAll('a', attrs={'season-episode-title'}) # recherche de toutes les balises <a> dans section 'season-episode-title'
        
        for div in data2:
            parsed_url = urlparse(url_no_loc+serie[0]) # ParseResult(scheme='https', netloc='transcripts.fandom.com', path='/wiki/The_Walking_Dead_(TV_series)', params='', query='', fragment='')
            url_page_script = parsed_url._replace(path=str(div['href']))._replace(query="") # Modifie le dernier element de l'url par le nom de l'episode 
            ...
            html_script = urllib.request.urlopen(new_url) # Parsing du lien ou se trouve la transcription
            soup_page_script = BeautifulSoup(html_script, 'html.parser')
            data_script = soup_page_script.findAll('div',attrs={'class':'scrolling-script-container'}) # Se placer dans le div ou il y a le script
            
            # Obtention du format SaisonX.EpisodeX.txt
            idEpisode = url_page_script.path.split("=")[2] # s01e01
            newIdEpisode = re.sub(r"(s)([0-9]{2,})(e)([0-9]{2,})", r"Season\2.Episode\4", idEpisode) # Season01.Episode01
            ...
            for div in data_script:
                ...
                with codecs.open(html_path / f"{serie[1]}.{newIdEpisode}.temp",'w', "utf8") as f:
...
def extractFandomWebsite():
    ...
    for div in data:
        links = div.findAll('a')
        for a in links:
            parsed_url = urlparse(url_loc) # ParseResult(scheme='https', netloc='transcripts.fandom.com', path='/wiki/The_Walking_Dead_(TV_series)', params='', query='', fragment='')
            url_page_script = parsed_url._replace(path=str(a['href'])) # Modifie le dernier element de l'url par le nom de l'episode 
            new_url = url_page_script.geturl() # Nouvel url pour acceder a la transcription de l'episode
            html_script = urllib.request.urlopen(new_url) # Parsing du lien ou se trouve la transcription            
            soup_page_script = BeautifulSoup(html_script, 'html.parser')
            data_script = soup_page_script.findAll('div',attrs={'class':'mw-content-ltr mw-content-text'}) # Se placer dans le div où il y a le script

            idEpisode = str(div.text.split(' ')[0]+div.text.split(' ')[1]).split('\n')[1]
            newIdEpisode = re.sub(r"(Season)([0-9]{1,})", r"Season0\2", idEpisode) # Season01.Episode01
            
            for div in data_script:
                transcript_path = DATA_PATH / "TheWalkingDead" / "transcripts"
                transcript_path.mkdir(exist_ok=True)
                with codecs.open (transcript_path/"TheWalkingDead"+newIdEpisode+".temp",'w', "utf8") as f:
                    f.write(str(div)) 
...
def extractForeverDreamingWebsite():
    ...
    for i in indexPage:
        ...
        soup_page_web = BeautifulSoup(html_page, 'html.parser') # parsing du html avec bs4
        data = soup_page_web.findAll('a', attrs={'topictitle'}) # recherche de toutes les balises <a> dans section 'season-episode-title'
        ...
        for div in data:
            if not "Online Store" in div.text and not "Board Updates:" in div.text:
                ...
                url_page_script = parsed_url._replace(path=extension_url_script)._replace(query="") # Modifie le dernier element de l'url par le nom de l'episode 
                ...
                if not "/" in idEpisode :
                    newIdEpisode = re.sub(r"([0-9]{2,})(x)([0-9]{2,})", r"Season\1.Episode\3", idEpisode) # Season01.Episode01
                else:
                    newIdEpisode = re.sub(r"([0-9]{2,})(x)([0-9]{2,})(\/)([0-9]{2,})", r"Season\1.Episode\3.\5", idEpisode) # Season01.Episode01

                for div in data_script:
                    transcript_path = DATA_PATH / "TheOffice" / "transcripts"
                    transcript_path.mkdir(exist_ok=True)
                    with codecs.open (transcript_path/ "TheOffice."+newIdEpisode+".temp",'w', "utf8") as f:
                        f.write(div.text) 
