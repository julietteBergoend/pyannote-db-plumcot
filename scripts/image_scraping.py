...
def get_url_from_character_page(url_IDMB,
...
def get_image_jsons_from_url(media_viewer_url, old_image_jsons=None,
    ...
    str_script = str_script.replace("'mediaviewer'",
                                    '"mediaviewer"')  # replace simple with double quotes
    json_begins_at = str_script.find("{")
    str_script = str_script[
                 json_begins_at:-1]  # the last character is ";" for some reason so we discard it
...
def query_image_from_json(image_jsons, IMAGE_PATH, actor2character, SEPARATOR=",",
                          IMAGE_FORMAT="jpg"):
    characters = {character: {"count": 0, "paths": []} for character in
                  actor2character.values()}  # counts the number of pictures per character
    ...
    print(
        f"there were {n_key_error_messages} images which had a problem mapping caption to a character :",
        end="")
...
def main(SERIE_URI, SERIE_IMDB_URL, IMAGE_PATH, CHARACTERS_PATH, N_COL, SEPARATOR,
         IMAGE_FORMAT):
